/*!
 * gulp
 * $ npm install gulp gulp-ruby-sass gulp-cached gulp-uglify gulp-rename gulp-concat gulp-notify gulp-filter gulp-jshint gulp-rev-append gulp-cssnano gulp-imagemin browser-sync gulp-file-include gulp-autoprefixer del gulp-plumber --save-dev
 */
...
gulp.task('testLess', function () {
    gulp.src('src/less/**/*.less', {style: 'expanded'})// 传入 less 目录及子目录下的所有 .less 文件生成文件流通过管道并设置输出格式
        .pipe(plumber({errorHandler: notify.onError('Error: <%= error.message %>')})) //报错提示
...
gulp.task('styleReload', ['testLess', 'css'], function() {
...
// build，关连执行全部编译任务
gulp.task('build', ['less', 'css', 'script', 'image'], function () {
  gulp.start('html');
});
